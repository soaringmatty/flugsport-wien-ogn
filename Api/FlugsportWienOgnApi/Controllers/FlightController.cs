...
namespace FlugsportWienOgnApi.Controllers
{
    ...
    public class FlightController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<ActionResult<IEnumerable<Flight>>> GetFlights([FromQuery] double? maxLat, [FromQuery] double? minLat, [FromQuery] double? maxLng, [FromQuery] double? minLng)
        {
            var flights = await _flightService.GetFlights(maxLat, minLat, maxLng, minLng);
            if (flights != null)
            {
                return Ok(flights);
            }
